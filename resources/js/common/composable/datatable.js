...
const datatable = () => {
    ...
    const queryData = (params) => {
        ...
        const url = generateUrl(tableUrl.value.url, limit, offset);
        sendingUrl.value = url;
        return axiosAdmin.get(url);
    };

    const getUrlWithQueryStringFromUrl = (url) => {
        const limit = table.pagination.pageSize;
        const offset = (currentPage.value - 1) * limit;

        return generateUrl(url, limit, offset);
    };

    const generateUrl = (url, limit, offset) => {
        var filterString = "";
        var hashableString = "";
