<template>
    <div
        v-if="
            permsArray.includes('email_providers_create') || permsArray.includes('admin')
        "
    >
        <a-tooltip
            v-if="tooltip"
            placement="topLeft"
            :title="$t('email_provider.add')"
        ...
        <AddEdit
            ...
            :formData="formData"
            :data="formData"
            :pageTitle="$t('email_provider.add')"
            :successMessage="$t('email_provider.created')"
        />
    </div>
</template>
<script>
